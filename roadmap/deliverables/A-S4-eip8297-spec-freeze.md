...
## Objective
Freeze the final EIP-8297 as the H\* Considered-for-Inclusion (CFI) spec: keys, node types, hash function `H`, and gas constants all fixed, with no remaining open protocol parameters. The freeze gives client teams a **stationary target** to build against through the conformance, devnet, and migration rehearsals that run toward fork S. This is the H\* milestone deliverable for the Trie Design thread.
